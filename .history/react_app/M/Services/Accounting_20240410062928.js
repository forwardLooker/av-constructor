  ...
  async componentDidMount() {
    ...
    operations.forEach(op => {
      op['Проводки'].forEach(pr => {
        pr['Дебет'].name
        let account = accounts.find(acc => acc.name === pr['Дебет'].name);
        if (!account) {
          account = {
            'Cчёт': pr['Дебет'].name,
            'Обороты за период': {
              'Дебет': pr['Аналитические парметры табличные'].reduce((accAllTabl, table) => {
                table['Аналитические парметры'].reduce(acc)
              }, 0)
            }
          }
        }
