  ...
  async componentDidMount() {
    ...
    operations.forEach(op => {
      op['Проводки'].forEach(pr => {
        ...
        let accountForDebit = accounts.find(acc => acc['CxСчёт'] === debitAccName);
        if (!accountForDebit) {
          accountForDebit = {
            'Cчёт': pr['Дебет'].name,
            'Обороты за период': {
        ...
        let accountForCredit = accounts.find(acc => acc['Счёт'] === creditAccName);
        if (!accountForCredit) {
          accountForCredit = {
            'Cчёт': pr['Кредит'].name,
            'Обороты за период': {
          ...
          accountForDebit['Обороты за период']['Кредит'] = accountForDebit['Обороты за период']['Кредит'] + additional;
        }
